...
public final class Fabric1201MainEntrypoint implements ModInitializer {
    ...
    private static void registerCommands(CommandDispatcher<CommandSourceStack> dispatcher) {
        ...
        var waypoint = Commands.literal("waypoint").then(list).then(add)
                .then(Commands.literal("remove").then(Commands.argument("id", UuidArgument.uuid()).executes(context -> new MapFabric1201Bootstrap().serverController().executeWaypointCommand(
                ...
                .then(Commands.literal("follow").then(Commands.argument("id", UuidArgument.uuid()).executes(context -> new MapFabric1201Bootstrap().serverController().executeWaypointCommand(
                        context.getSource().getEntityOrException().getUUID(), new DimensionId(context.getSource().getLevel().dimension().location().toString()),
