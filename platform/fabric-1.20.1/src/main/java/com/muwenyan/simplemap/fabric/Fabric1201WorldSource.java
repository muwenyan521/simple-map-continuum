...
public final class Fabric1201WorldSource implements WorldSourcePort, SurfaceColumnSourcePort, CaveColumnSourcePort {
    private final Supplier<ClientLevel> level;
    private final AtomicLong revision = new AtomicLong();

    public Fabric1201WorldSource(Supplier<ClientLevel> level) {
    ...
    public Optional<SurfaceColumn> sample(ChunkPos chunk, int localX, int localZ) {
        if (localX < 0 || localX >= 16 || localZ < 0 || localZ >= 16) {
            throw new IllegalArgumentException("local coordinates out of range");
        }
        ClientLevel current = level.get();
    ...
    public List<CaveColumnRun> sample(ChunkPos chunk, int localX, int localZ, CaveConfig config) {
        ...
        for (int y = Math.min(top, config.topY()); y >= current.getMinBuildHeight(); y--) {
            ...
            if (!state.isAir() && state.isSolidRender(current, position)) {
                ...
                return List.of(new CaveColumnRun(y, y, color, current.getBrightness(net.minecraft.world.level.LightLayer.BLOCK, position),
                        !state.getFluidState().isEmpty(), state.getLightEmission() > 0));
            }
        }
        return List.of();
    }
