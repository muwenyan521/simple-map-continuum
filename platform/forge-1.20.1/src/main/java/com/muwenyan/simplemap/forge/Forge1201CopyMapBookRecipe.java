...
public final class Forge1201CopyMapBookRecipe extends CustomRecipe {
    public Forge1201CopyMapBookRecipe(ResourceLocation id, CraftingBookCategory category) { super(id, category); }
    @Override public boolean matches(CraftingContainer input, Level level) {
        return MapBookCrafting.matchesCopy(states(input));
    }
    ...
    @Override public net.minecraft.world.item.crafting.RecipeSerializer<?> getSerializer() { return Forge1201Recipes.COPY.get(); }
    private static List<MapBookItemState> states(Container input) {
        ...
        for (int index = 0; index < input.getContainerSize(); index++) {
            ...
            else if (stack.is(Forge1201Entrypoint.EMPTY_MAP_BOOK.get())) states.add(MapBookItemState.empty());
            else return List.of();
        }
